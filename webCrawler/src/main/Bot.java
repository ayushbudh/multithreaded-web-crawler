...
//defines the 'Bot' class which implements the Runnable interface indicating that instances of this class can be executed by threads
public class Bot implements Runnable {
    ...
    //constructor
    public Bot(String startLink, int maxDepth, int numThreads) {
        System.out.println("webCrawler created");
        //initializes the visitedLinks set and uses collections.synchronizedSet to make the set thread-safe
    ...
    public void startBot() {
    	//loads the previously visited links from the state file, updating the visitedLinks set
        loadState();
        
        //an array of thread objects is created to manage multiple threads
        Thread[] threads = new Thread[numThreads];
        // for each thread a new Thread object is created with the current instance of Bot
        for (int i = 0; i < numThreads; i++) {
            threads[i] = new Thread(this);
            //initiates the execution of the thread which invokes the run method
            threads[i].start();
        }
        // this loop iterates over each thread in the array and overall ensures that the program waits for all threads 
    ...
    //synchronized keyword ensures that only one thread can execute this method at a time which makes it thread safe
    private synchronized void crawl(int level, String url) {
    	//condition that checks whether the current depth is within the specified maximum depth and if the current URL has not been visited
        if (level <= maxDepth && !visitedLinks.contains(url)) {
        	/*using the Jsoup library the request method retrieves the web page content as a Document object
        	 * this method makes an HTTP request to the given URL and returns the parsed HTML document
        	 */
            ...
            if (doc != null) {
                ...
                for (Element link : doc.select("a[href]")) {
                    ...
                    //crawl method is recursively called with an incremented depth and the next link
    ...
    private Document request(String url) {
        ...
        try {
            ...
            /*the get method sends an HTTP GET request to the URL and parses the response into a Document object
             * so basically the get method retrieves the HTML documents from the connected URL
             */
            Document doc = con.get();
            //condition that checks if the HTTP request is successful
            if (con.response().statusCode() == 200) {
            	//message is printed that shows that the web page was successfully received by the bot
                System.out.println("\n Bot ID:" + Thread.currentThread().getId() + " Received Webpage at " + url);
    ...
    private void loadState() {
    	 try {
             ...
             if (Files.exists(filePath)) {
            	 //reads all lines from the file and returns them as a list and the list is used to create a hashset names loadedLinks
                 Set<String> loadedLinks = new HashSet<>(Files.readAllLines(filePath));
                 //the loadedLinks elements are added to the existing visitedLinks set which ensures that the bot's visitedLinks is updated with the loaded data
